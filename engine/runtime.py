...
class RuntimeConfig:
    ...
    npc_self_update_interval: int = 5  # Every N turns, NPC updates their card
    ...
    worldbuilder_model: str = "deepseek/deepseek-v4-pro"  # Model for worldbuilder calls (compaction, create_*, item_expand)
...
def compact_memory(
    ...
    model: str,
    keep_recent: int = 3,
    temperature: float = 0.7,  # summarization wants stability without flatness (per Maciej)
) -> dict[str, Any]:
    ...
    # Call LLM
    raw = llm_call(MEMORY_COMPACTION_SYSTEM, user_prompt, model, temperature)
...
def run_round(
    ...
    # slow oracle just delivered.
    pending_weaver: Future | None = getattr(engine.state, "_pending_weaver", None)
    if pending_weaver is not None and pending_weaver.done():
        try:
            raw_weaver = pending_weaver.result(timeout=0)
    ...
    if llm_actors and npc_decider is not None:
        with ThreadPoolExecutor(max_workers=min(8, len(llm_actors))) as pool:
            futures = {
                pool.submit(npc_decider, a, prompts_at_round_start[a["id"]]): a["id"]
                for a in llm_actors
            }
            for fut in futures:
                aid = futures[fut]
                try:
                    raws_by_id[aid] = fut.result()
                except Exception as exc:
                    audit.append(f"npc_decide_failed:{aid}:{type(exc).__name__}")
                    raws_by_id[aid] = ""  # treat as wait
    ...
    for actor in actors:
        ...
        if npc_decider and npc_should_use_llm(engine, actor, player, config.llm_activation_radius):
            # Use pre-fetched raw decision from parallel fan-out above (Proposal 1).
            # Fallback to direct call only if pre-fetch missed (shouldn't happen).
            if actor_id in raws_by_id:
                raw = raws_by_id[actor_id]
            else:
                prompt = engine.build_prompt(actor_id)
                raw = npc_decider(actor, prompt)
            parsed = parse_npc_action(raw) if raw else {"action": {"verb": "wait"}}
            ...
            # Public action lines are emitted by each rule's message effect.
            # No fallback log here — that path duplicated what the rule already said.

            # Compaction is hoisted to a parallel post-loop block (Proposal 2).
            # Trigger now uses overflow-only (Proposal 6) — periodic-by-turn was
            # busting the cache prefix unnecessarily.
        else:
    ...
    # --- Parallel compaction fan-out (Proposals 2 + 6) --------------------
    # Overflow-only trigger: fires when a being's private_log actually
    # exceeds the token limit. No periodic schedule — that was busting the
    # cache prefix unnecessarily. All eligible NPCs compact concurrently.
    candidates_for_compaction: list[str] = []
    for actor in actors:
        actor_id = actor["id"]
        if actor_id == player_id:
            continue
        private_log = actor.get("private_log", [])
        if len(private_log) <= 5:
            continue
        log_tokens = sum(count_tokens_text(e.get("text", "")) for e in private_log)
        if log_tokens > config.npc_self_update_token_limit:
            candidates_for_compaction.append(actor_id)

    if candidates_for_compaction:
        llm = _worldbuilder_llm(config)
        if llm:
            def _compact_one(aid: str):
                return aid, compact_memory(engine, aid, llm, config.worldbuilder_model)
            with ThreadPoolExecutor(max_workers=min(8, len(candidates_for_compaction))) as pool:
                for aid, result in pool.map(_compact_one, candidates_for_compaction):
    ...
    # --- Weaver (GM_ANTERIOR) — async dispatch (Proposal 3) ---------------
    # The weaver is the slow oracle. Don't wait for it. Fire it on a
    # background single-thread executor; its results land at the start of
    # whatever turn it completes by (handled at the top of run_round).
    if (
        config.weaver_enabled and weaver_decider
        ...
        if existing is None or existing.done():
            ...
            if pool is None:
                pool = ThreadPoolExecutor(max_workers=1, thread_name_prefix="weaver")
                engine.state._weaver_pool = pool
            engine.state._pending_weaver = pool.submit(weaver_decider, weaver_prompt)
            audit.append(f"weaver_dispatched:turn={engine.state.turn}")
            if step_callback:
