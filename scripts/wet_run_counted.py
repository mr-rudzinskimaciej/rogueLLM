...
DISABLE_THINKING: list[bool] = [False]  # legacy global; superseded by THINKING_BY_ROLE below

# Per-role thinking budget — Maciej's split. NPCs/breath are constrained
# tasks where reasoning is theatrical latency. Weaver, settling, worldbuilder
# benefit from thinking. User explicitly requested weaver keep thinking ON.
THINKING_BY_ROLE: dict[str, bool] = {
    "npc":            False,
    "gm_breath":      False,
    "gm_settling":    True,
    "weaver":         True,   # the slow oracle — keep thinking
    "worldbuilder":   True,
    "npc_compact":    True,   # summarization deserves care, just not creativity
}

# Per-role temperature override (proposal 5, with user-requested 0.7 for compaction).
# Falls back to args.temp / args.temp_gm if role not listed.
TEMP_BY_ROLE: dict[str, float] = {
    "gm_settling":    1.0,    # post-round resolution wants slight stability
    "weaver":         1.4,    # long-arc imagination — go higher
    "worldbuilder":   1.0,
    "npc_compact":    0.7,    # summarization: stable but not flat (per user)
}
...
def instrument(pricing: dict[str, float] | None) -> None:
    ...
    def _wrapped(system_prompt: str, user_prompt: str, model: str,
        ...
        # Per-role overrides for thinking budget and temperature
        role = CURRENT_ROLE[0]
        role_thinking = THINKING_BY_ROLE.get(role, not DISABLE_THINKING[0])
        role_temperature = TEMP_BY_ROLE.get(role, temperature)
        for attempt in range(max_retries + 1):
            try:
                extra: dict[str, Any] = {"usage": {"include": True}}
                if not role_thinking:
                    extra["reasoning"] = {"enabled": False}
                provider_pin = adapter._provider_extra_body(current)
                if provider_pin:
                    extra.update(provider_pin)
                t0 = _time.monotonic()
                ...
                adapter.call_log.append({
                    "role": CURRENT_ROLE[0], "model": current, "ok": True,
                    "raw_len": len(text or ""),
                ...
                adapter.call_log.append({
                    "role": CURRENT_ROLE[0], "model": current, "ok": False,
                    "raw_len": 0,
                    "elapsed_ms": int((_time.monotonic() - t0) * 1000) if 't0' in locals() else 0,
    ...
    for turn_index in range(args.turns):
        ...
        if args.enable_gm:
            try:
                ...
                print(f"  [breath error: {type(exc).__name__}: {exc}]")
        round_audit: list[str] = []
        try:
            round_audit = run_round(
                engine=engine,
                player_id=args.player,
        ...
        if capture_enabled:
            cost_audit = [f"role:{c['role']}:{c['total_tokens']}tok:${(c['cost_reported'] or c['cost_computed']):.4f}" for c in turn_calls]
